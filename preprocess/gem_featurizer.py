...
class GeoPredTransformFn(object):
    ...
    def __call__(self, raw_data):
        """
         Convert input data to molecules

        Args:
            raw_data: Can be a SMILES string or a dictionary containing SMILES

        Returns:
            Converted molecular data
        """
        if isinstance(raw_data, dict):
            smiles = raw_data.get('smiles')
            if smiles is None:
                print(f"Error: No ‘smiles’ key in the input dictionary:  {raw_data}")
                return None
        else:
            smiles = raw_data
        try:
            ...
            if mol is None:
                print(f"Unable to create molecule from SMILES: {smiles}")
                return None
            ...
            print(f"Error in processing molecule  ({smiles}): {str(e)}")
            return None
